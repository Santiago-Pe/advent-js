...
function decode(message) {
  // Find the innermost pair of parentheses and reverse its content
  const regex = /\(([^()]+)\)/;

  // Check if there are any parentheses left
  if (regex.test(message)) {
    // Replace innermost pair of parentheses with its reversed content
    message = message.replace(regex, (_, content) => {
      // Reverse the content inside the parentheses
      const reversedContent = content.split("").reverse().join("");
      return reversedContent;
    });

    // Recursively call decode function until there are no parentheses left
    return decode(message);
  } else {
    // If there are no more parentheses, return the decoded message
    return message;
  }
}
